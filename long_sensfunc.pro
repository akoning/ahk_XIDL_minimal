  ...
  if (nstd gt 1) then begin
     for ii = 0, nstd-1 do begin
        ...
        if (ii eq 0) then begin
           ...
           sz = size(wave)
           sz1 = size(wave1)
           if sz[1] ne sz1[1] then begin
              wave  = [[wave],[interpol(wave1,findgen(sz1[1]),findgen(sz[1]))]]
              flux = [[flux],[interpol(flux1,wave1,wave[*,0])]]
              ivar = [[ivar],[interpol(ivar1,wave1,wave[*,0])]]
              std_flux = [[std_flux],[interpol(std_flux1,wave1,wave[*,0])]]
           endif else begin
              wave = [[wave],[wave1]]
              flux = [[flux],[flux1]]
              ivar = [[ivar],[ivar1]]
...
; Jan 16, 2015: AHK changed to read in opt tags instead of box
wave = standard_struct.wave_opt
flux = standard_struct.flux_opt
ivar = standard_struct.ivar_opt
