...
;     Read this file
       ...
       ELSE long_proc, filenames[ifile] $
                       ...
                       , verbose = verbose 
       if (size(hdr1))[0] gt 1 then hdr2 = reform(hdr1,n_elements(hdr1)) else hdr2 = hdr1
       telescope = strcompress(sxpar(hdr2, 'TELESCOP'), /rem)
       ...
       if ifile EQ 0 then begin
          if (stregex(sxpar(hdr2,'INSTRUME'),'.*kast.*',$
                         /boolean,/fold_case) eq 1) or $
             (stregex(sxpar(hdr2,'VERSION'),'kast*', /bool, /fold_case) EQ 1) or $
             (stregex(sxpar(hdr2,'INSTRUME'),'.*mmtbluechan.*',$
                      /boolean,/fold_case) eq 1) then $
                         kast = 1 else kast = 0
...
;        Default to all mask values of 1B, which rejects points
           IF n_pixel GT 0 THEN BEGIN
               ...
               inmask = make_array(dimension = [dims, n_pixel], /byte $
                                   , value = 1B)
           ENDIF
           sxaddpar, hdr[*,0], 'NEXP', nfile, 'Number of exposures in this file', $
                    before = 'EXPTIME'
      endif else begin
    ...
    sxaddpar, hdr[*,0], string(ifile+1, format = '("FILE",i2.2)'), $
