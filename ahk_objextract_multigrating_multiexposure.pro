...
ahk_profile_singlegrating,scifileblue[0],slitfile=slitfileblue,wavefile=wavefileblue
ahk_profile_singlegrating,scifilered9[0],slitfile=slitfilered9,wavefile=wavefilered9
ahk_profile_singlegrating,scifilered4[0],slitfile=slitfilered4,wavefile=wavefilered4
...
FOR jj = 0 , nexpred9-1 DO BEGIN 
   FOR ii =1,nslitred9 DO BEGIN
	search = file_search('normprofile_'+strmid(scifilered9[0],15,8,/reverse_offset)+'_slitid'+StrTrim(ii,2)+'.csv',count=c1)
	;; If CSV file exists, and therefore there is flux in that slit, then continue with the analysis. Otherwise skip to next slit.
	IF (c1 LT 1) THEN CONTINUE
		print, '***** Working on RED9 EXPOSURE ', jj, ', SLITID ', ii, ' *****'
...
FOR jj = 0 , nexpred4-1 DO BEGIN 
   FOR ii =1,nslitred4 DO BEGIN
	search = file_search('normprofile_'+strmid(scifilered4[0],15,8,/reverse_offset)+'_slitid'+StrTrim(ii,2)+'.csv',count=c1)
	;; If CSV file exists, and therefore there is flux in that slit, then continue with the analysis. Otherwise skip to next slit.
	IF (c1 LT 1) THEN CONTINUE
		print, '***** Working on RED4 EXPOSURE ', jj, ', SLITID ', ii, ' *****'
