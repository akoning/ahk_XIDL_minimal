...
;
     ...
     ; peakval = 1
      ...
      ;; Find candidate ending positions
      xmax = long_find_nminima(fsum, nfind = 1, minsep = minsep, width = 3)
      ;peakcut = peakthresh*interpolate(fsum, xmax) ;Original theshold
      peakcut = -5d5 ;Threshold chosen by AHK
      inoise = where(abs(fsum) lt abs(peakcut))  ;HE      
      ftmp = fsum & ftmp[inoise] = 0.
      ...
      if (nend GT 0) then begin
         ...
         ;igood = where(fsum[xend] LT -abs(peakcut), ngood);where(fsum[xend] LT -peakthresh*peakval, ngood) ;Changed by AHK
      ...
      ;;ALICE'S TEST TO MAKE SURE NO SLITS ARE MISSED:
      ;;IF THERE ARE TWO STARTS FOLLOWED BY TWO ENDS (WHICH SEEMS TO BE WHAT
      ;;HAPPENS SOMETIMES WHEN TWO SLITS GET COMBINED INTO ONE),
      ;;ADD AN END 2 BEFORE THE SECOND START, AND A START 2 AFTER THE FIRST END.
      ;;FIRST THOUGH, CHECK FOR THREE STARTS IN A ROW, FOLLOWED BY ONE END.
      ;;IF THIS EXISTS, CHANGE THE THIRD START TO AN END, SO IT BECOMES
      ;;TWO STARTS FOLLOWED BY TWO ENDS
      
      kgood = where( (allq EQ 1 and allq EQ shift(allq, -1) and allq EQ $
                      shift(allq, -2) and allq NE shift(allq, -3) and allq EQ shift(allq, -4)), nkgood)
      if nkgood gt 0 then begin
         FOREACH kval, kgood, ct DO BEGIN
            allq[kval+2] = -1
         ENDFOREACH
      endif
      ...
      if njgood gt 0 then begin
         allx1=allx
         FOREACH jval, jgood, ct DO BEGIN
            allq = arrinsert(allq,-1,at=ct*2+jval+1)
            allq = arrinsert(allq,1,at=ct*2+jval+4)
            allx = arrinsert(allx,allx1[jval+1]-2,at=ct*2+jval+1)
            allx = arrinsert(allx,allx1[jval+2]+2,at=ct*2+jval+4)
         ENDFOREACH
         ...
         ;; AHK fix for a missing start and end that is otherwise
         ;; undetectable: Look for consecutive end/start that are
         ;; greater than minslit apart. If this occurs, add a start
         ;; right after the end and an end right before the
         ;; start. This will change quite a bit though... so not going
         ;; to add in for real...
      ;hgood = where( (allq EQ -1 and allq NE shift(allq, -1)  and (shift(allx,-1)-allx) GE 50), nhgood)
