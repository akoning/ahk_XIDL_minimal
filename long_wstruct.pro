...
;   7-Jan-2015   AHK changed wstruct.psig and wstruct.sigrej for LRIS '400/8500'
;-
;------------------------------------------------------------------------------
...
if (instrument EQ 'LRISBLUE') then begin
    ...
    CASE grism OF
        ...
        '600/4000': begin
            ;; parameters for arc peak finding
            wstruct.pkwdth        = 3.0D/binning[1] ; Original: 12.0D/binning[1]
            wstruct.TOLER         = 2.0D/binning[1] ; Original:  4.0D/binning[1]
            wstruct.psig[0:5]     = [12.0, 12.0, 10.0, 10.0, 10.0, 10.0]*0.5; original: [12.0, 8.0, 7.0, 4.0, 4.0]
            wstruct.mxoff[0:5]    =  [7.0, 7.0, 5.0, 5.0, 5.0, 5.0]/binning[1] ;Original: [7.0, 7.0, 5.0, 5.0, 5.0]/binning[1]
            ;; parameters for wavelength soln fits
            ;wstruct.sigrej[0:4]   = [3.0, 3.0, 3.0, 3.0, 3.0]
            wstruct.sigrej[0:5]   = [3.0, 3.0, 2.5, 2.5, 2.0, 2.0] ;Original: [3.0, 3.0, 3.0, 2.5, 2.5]
            wstruct.nord[0:5]     = [4L, 4L, 5L, 5L, 5L, 5L] ;Original: [4L, 5L, 5L, 5L, 5L]
            wstruct.FLG_QUAL[0:5] = [1L, 1L, 1L, 2L, 2L, 2L] ;Original: [1L, 1L, 1L, 2L, 2L]
            wstruct.LINELIST      = line_path+'/lris_blue_600_AHK.lst' ;Original: line_path+'/lris_blue_600.lst'
            wstruct.npanic        = 5L
            wstruct.nord_panic    = 2L
            wstruct.REID          = 1 ; reidentify using archived solutions
            wstruct.REID_FILE     = calib_path +'/lris_blue_600_ahk_calibration_set.sav' ;Original: calib_path + '/lris_blue_600.sav'
            wstruct.BIN_RATIO     = float(binning[1]);Original: float(binning[1])/2L ;arxiv soln binned spectrally
            ;;AUTOID parameters hacked in by AHK
;            wstruct.nfind        = 30L ; Original value: 30L
;            wstruct.disp_guess   = 0.635*binning[1]
            wstruct.FUNC  = 'CHEBY'
;            wstruct.dr_wave       = [0.9, 1.1]*(0.635d0)*binning[1]
;            wstruct.AUTOID        = 0
        END
        '400/3400': begin
           ;; Query on dichroic
...
ENDIF ELSE IF (instrument EQ 'LRIS') THEN BEGIN
    ...
    CASE grating OF
        ...
        '400/8500': BEGIN
            ...
            wstruct.psig[0:5]  = [10.0, 10.0, 10.0, 10.0, 10.0, 10.0]
            ; first alternative psig values: [10.0, 7.0, 7.0, 5.0, 4.0, 4.0] ; default psig values: [7.0, 7.0, 7.0, 5.0, 4.0, 4.0]
            wstruct.MXOFF[0:5] = [3.0, 3.0, 3.0, 3.0, 3.0, 2.0]/binning[1]
            wstruct.sigrej[0:5]   = [3.0, 3.0, 2.0, 2.0, 2.0, 2.0] ; default sigrej values: [4.0, 4.0, 3.0, 3.0, 3.0, 3.0]
            wstruct.nord[0:5]     = [4L, 4L, 5L, 5L, 5L, 5L]
            wstruct.FLG_QUAL[0:5] = [1L, 1L, 2L, 2L, 2L, 2L]
            wstruct.LINELIST      = line_path+'/lris_red_300_AHK400.lst' ;'/lris_red_300.lst'
            wstruct.npanic        = 10L
            wstruct.nord_panic    = 2L
            if strmid(sxpar(hdr,'DATE'),10) LT '2009-07-01' then begin
               wstruct.REID        = 1
               wstruct.REID_FILE   = calib_path + $
                                     'lris_red_400_ahk_calibration_set.sav'
                                     ;'/lris_red_400_grangle_24.32_multi.sav'
;;           These are AUTOID paramaters that should be used for a
;;           a different grangle.
               wstruct.nfind        = 100L ; Original value: 30L
...
;            wstruct.FLG_QUAL[0] = 2L
        ...
        '900/5500': BEGIN
            ...
            wstruct.MXOFF[0:5]    = [6.0, 6.0, 6.0, 5.0, 3.0, 2.0]/binning[1]
            wstruct.sigrej[0:5]   = [3.0, 3.0, 3.0, 3.0, 3.0, 3.0]
            wstruct.nord[0:5]     = [3L, 3L, 4L, 4L, 5L, 5L];Original: [3L, 3L, 4L, 4L, 4L, 4L]
            wstruct.FLG_QUAL[0:5] = [1L, 1L, 1L, 2L, 2L, 2L]
            wstruct.LINELIST        = line_path+'/lris_red_600_AHK.lst';Original:  line_path+'/lris_red_600.lst'
            wstruct.REID        = 1
